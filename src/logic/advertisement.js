...
const insertAdvertisement = async (request, response, pool) => {
  try {
    ...
    await pool.query(
      "INSERT INTO advertisement ( video_link, date, duration, status, question, answers, correct_answer, filled_answer) values ($1, $2, $3, $4, $5, $6, $7, $8)",
      [ video_link, date, duration, status, question, answers, correct_answer, filled_answer]
    );
    return response.status(200).json({
      message: "success",
      token: request.token,
